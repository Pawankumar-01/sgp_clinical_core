...
const FASTAPI_BASE = "http://localhost:8001/api/v1";
  ...
  refresh(frm) {
    ...
    if (frm.doc.room_name) {
      frm.add_custom_button("🩺 Join as Host", () => {
        const hostUrl = `http://localhost:8001/meet/host.html?session=${frm.doc.room_name}&name=${encodeURIComponent(frappe.session.user)}`;
    ...
    if (frm.doc.room_name) {
      frm.add_custom_button("🔗 Copy Patient Join Link", () => {
        const link = `http://localhost:8001/meet/index.html?session=${frm.doc.room_name}`;
    ...
    if (frm.doc.room_name) {
      frm.set_intro(
        `<b>Session ID:</b> ${frm.doc.room_name}<br>
         <b>Patient Join:</b> http://localhost:8001/meet/index.html?session=${frm.doc.room_name}<br>
         <b>Host Join:</b> http://localhost:8001/meet/host.html?session=${frm.doc.room_name}`,
        "blue"
      );
    }
  },
