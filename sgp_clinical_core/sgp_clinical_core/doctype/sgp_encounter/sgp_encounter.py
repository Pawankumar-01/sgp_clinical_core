...
# ══════════════════════════════════════════════════════════════════════════════
# Patient Clinical History — Whitelisted Server Method
# Called from sgp_encounter.js when the `patient` field changes.
# ══════════════════════════════════════════════════════════════════════════════

@frappe.whitelist()
def get_patient_encounter_history(patient, limit=5):
    """
    Returns the last N *submitted or approved* SGP Encounters for a given patient
    as a list of plain dicts. Used to populate the read-only Patient History tab
    in the SGP Encounter form without any additional page load.

    Args:
        patient (str): ERPNext Patient docname (e.g. "PAT-00001").
        limit   (int): Maximum number of past encounters to return. Default 5.

    Returns:
        list[dict]: Encounter snapshots ordered newest-first.
    """
    if not patient:
        return []

    try:
        limit = max(1, min(int(limit), 20))  # guard: 1–20 range
    ...
    encounters = frappe.get_list(
        ...
        fields=[
            "name", "encounter_date", "doctor",
            "status", "case_type", "review_after",
            # Chief complaint / anamnesis
            "chief_complaint", "anamnesis",
            # Vitals
            "height_cm", "weight_kg", "bp", "pr", "rr", "temp",
            "wrist_cm", "waist_cm", "hip_cm",
            # Examination
            ...
            # Treatment
            "sgp_rx", "panchakarma", "detox_procedures", "allopathic_medicines",
            # Diet & lifestyle
            "diet_include", "diet_exclude", "exercises_yoga",
            "lifestyle_advice", "home_remedies",
            # Follow-up
            "follow_up", "prognosis",
        ],
    ...
    # Exclude the current (in-progress) encounter from history if it has been
    # saved — it will have the same patient but no name yet on a brand-new form.
